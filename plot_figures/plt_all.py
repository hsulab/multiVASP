...
def main():
    mix_file = os.path.join(os.path.expanduser('~'), 'Desktop/mix_data_20180514.csv')
    fig_file = os.path.join(os.path.expanduser('~'), 'Desktop/mix_data.png')
    ###
    mix_df = pd.read_csv(mix_file, index_col=0)
    ### Plot colored
    paras = []
    for para in mix_df.columns:
        if not para in ['cell', 'dop', 'name']:
            paras.append(para)
    print(paras)
    sns.pairplot(mix_df, vars = paras, hue = 'cell', \
            plot_kws = {'alpha': 0.6, 's': 80, 'edgecolor': 'k'}, size = 4)
    # Title
    plt.suptitle('TS-EDA', size = 28);
    plt.legend()
