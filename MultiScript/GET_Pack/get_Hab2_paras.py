...
###
def get_Hab2_paras(abc, l1, l2, Hab, cif_name):
    ...
    if 'pure' in cif_name:
        ...
        name = cif_name.split('_')[0] + '_' + cif_name.split('_')[1] + '_Hab2'
    ...
    ##
    Hab2_paras = [cell, dop, name, dH1_O2, dO2_M4, dM4_l2O2, \
            aH1_O2_M4, aO8_O2_H1]
    return Hab2_paras
...
###
def main():
    cons_dir = './data/Hab_data/Hab2_cons'
    cifs_dir = './data/Hab_data/Hab2_cifs'
    ###
    MS_dir = os.path.join(os.path.expanduser('~'), 'Documents/USRP/DopRutile_Files/Documents')
    MS_Hab2_cifs_dir = os.path.join(MS_dir, 'Hab2_cifs')
    ###
    Hab2_csv_name = 'Hab2_data_' + time.strftime("%Y%m%d", time.localtime()) + '.csv'
    Hab2_csv = os.path.join(os.path.expanduser('~'), 'Desktop/' + Hab2_csv_name)
    ### pd.DataFrame
    Hab2_df = pd.DataFrame(columns=('cell', 'dop', 'name', 'dH1_O2', \
    ...
    for con_name in os.listdir(cons_dir):
        ...
        ms_xsd = os.path.join(MS_Hab2_cifs_dir, os.path.basename(con)+'.xsd')
        ###
        abc, l1, l2, CH4, cif_name = std_cons(con)
        Hab2_paras = get_Hab2_paras(abc, l1, l2, CH4, cif_name)
        Hab2_df.loc[row_index] = Hab2_paras
        row_index += 1
        ##
        cif = os.path.join(cifs_dir, cif_name)
        if not os.path.exists(ms_xsd):
            ms_cif = os.path.join(MS_Hab2_cifs_dir, cif_name)
            shutil.copy(con+'.cif', cif)
            shutil.move(con+'.cif', ms_cif)
        else:
            os.remove(con+'.cif')
    ###
    Hab2_df = Hab2_df.sort_values(by=['cell','dop', 'name'], ascending=True)
    Hab2_df = Hab2_df.reset_index(drop=True)
    Hab2_df.to_csv(Hab2_csv)
###
