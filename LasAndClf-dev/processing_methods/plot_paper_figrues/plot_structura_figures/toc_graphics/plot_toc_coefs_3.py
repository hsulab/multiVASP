...
def plot_selected_features(ax, features, feanames):
    ...
    ax.set_yticklabels(['']*nfeatures, fontsize=10, fontweight='black')
    ax.set_ylabel(r'        $\bf{{{CH}_3}^*+H^*}$', \
            fontsize=16, horizontalalignment='left')
    ax.set_yticks([])

    ax.set_xlabel('Coefficient', fontsize=16, fontweight='bold')
    ax.set_xlim(0, 1.0)
    ax.set_xticks([])
    ...
    if '' in feanames:
        ...
        #title = 'Geometrical and Energetic Descriptors\n'
        title = r'$\bf{\stackrel{ss}{\longleftarrow}}$'
        title += r'     $\bf{{CH}_4}$     '
        title += r'$\bf{\stackrel{ra}{\longrightarrow}}$'
        ax.set_title(title, \
            fontsize=24, fontweight='bold')

        ax2 = ax.twinx()
        ax2.set_yticks([])
        ax2.set_ylabel(r'     $\bf{{{CH}_3}(g)+H^*}$', \
                fontsize=16, horizontalalignment='left')
    ...
    if '' in feanames:
        k=[0,0,0,0,0,0,0,0,0.14,0.2,0.08]
        for i, coef in enumerate(features.values()):
            ax.text(np.fabs(coef)+k[i], i, '%s' %(feanames[i]), \
                    fontsize=16, fontweight='bold')
        ax.set_axis_off()
    else:
        for i, coef in enumerate(features.values()):
            ax.text(np.fabs(coef), i, '%s' %(feanames[i]), \
